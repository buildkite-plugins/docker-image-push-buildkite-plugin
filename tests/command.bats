...
setup() {
  ...
  export BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE='test-image'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_TAG='test-tag'
}

@test "Fails if provider is missing" {
  unset BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER
...
@test "Fails if image is missing" {
  export BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER='ecr'
  unset BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE
  run "$PWD"/hooks/environment
  assert_failure
...
@test "ECR provider requires AWS CLI" {
  export BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER='ecr'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE='test-image'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_ECR_REGION='us-west-2'
  run "$PWD"/hooks/environment
  assert_failure
...
@test "GAR provider requires gcloud" {
  export BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER='gar'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE='test-image'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_GAR_PROJECT='test-project'
  run "$PWD"/hooks/environment
  assert_failure
...
@test "Fails on unsupported provider" {
  export BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER='unknown'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE='test-image'
  run "$PWD"/hooks/environment
  assert_failure
...
@test "Verbose mode enables debug output" {
  export BUILDKITE_PLUGIN_DOCKER_PUSH_PROVIDER='ecr'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_IMAGE='test-image'
  export BUILDKITE_PLUGIN_DOCKER_PUSH_VERBOSE='true'
  run "$PWD"/hooks/environment
